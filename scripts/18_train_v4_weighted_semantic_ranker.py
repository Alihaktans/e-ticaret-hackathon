...
RUN_NAME = "embedding_ranker_v4_weighted_semantic"
...
def build_train_frame() -> pd.DataFrame:
    print("Reading V4 train enriched...")
    ...
    train["sample_weight"] = train["negative_type"].map(SAMPLE_WEIGHTS).fillna(1.0).astype(np.float32)

    print("\nSample weight distribution:")
    print(train.groupby("negative_type")["sample_weight"].describe())
...
def make_topratio_submission(test_df: pd.DataFrame, ratio: float) -> Path:
    ...
    score_series = test_df["final_score"]
    ...
    out_path = SUBMISSIONS_DIR / f"{RUN_NAME}_e080_m020_topratio_{tag}.csv"
...
def main() -> None:
    ...
    print("=" * 100)
    print("18 TRAIN V4 WEIGHTED SEMANTIC RANKER")
    print("=" * 100)
    ...
    print("\nTrain negative type distribution:")
    print(tr["negative_type"].value_counts(normalize=True))
    print("\nValid negative type distribution:")
    ...
    train_pool = Pool(
        ...
        weight=tr["sample_weight"].astype(float),
    ...
    valid_pool = Pool(
        ...
        weight=va["sample_weight"].astype(float),
        cat_features=cat_indices,
    )
    ...
    test[["id", "term_id", "item_id", "embedding_score", "proba", "embedding_rank_score", "final_score"]].to_parquet(
        proba_path,
        index=False,
    )
    ...
    experiment_report = {
        ...
        "run_name": RUN_NAME,
        "model": "CatBoostClassifier on V4 semantic hard negatives with low semantic_hard sample weight",
        "task_type": task_type,
        "sample_weights": SAMPLE_WEIGHTS,
        "feature_cols": FEATURE_COLS,
        ...
        "warning": "Semantic hard negatives may include unlabeled positives; they are downweighted.",
    }
