...
ROOT = Path(".")
VAL = ROOT / "data/processed/v13_cross_encoder_validation_scores.parquet"
OUT = ROOT / "reports/manual_review/v13_banded_cross_encoder_validation.csv"
...
final_filter = (gender_mismatch | safe_brand_filter).to_numpy()
...
for label_set, part_idx in df.groupby("label_set").groups.items():
    ...
    ce = part["cross_sigmoid"].to_numpy()
    filt = final_filter[df.index.get_indexer(part.index)]

    # current baseline
    base = (v5 >= 0.60).astype(int)
    base[filt & (base == 1)] = 0
    row = {"label_set": label_set, "variant": "current_v5_0p6_gsb", "w_v5": 1.0, "threshold": 0.60, "n": len(part)}
    row.update(metrics(y, base))
    rows.append(row)
