...
def save_variant(variants, name, pred, ids):
    ...
    variants[name] = pred
    if len(variants) <= 10 or len(variants) % 100 == 0:
        print("kept", len(variants), name, "ones", int(pred.sum()), "pos_ratio", float(pred.mean()))
...
# Save only top candidates after evaluation, not every generated variant.
top_names = []
for col in ["public_safe_score", "combined_score", "legacy_weighted", "active_weighted", "main_min_macro"]:
    if col in agg.columns:
        for nm in agg.sort_values(col, ascending=False).head(10)["variant"].tolist():
            if nm not in top_names:
                top_names.append(nm)

for nm in ["v24_anchor", "v26_strict2000", "v26_supported2000", "v22_base_public080"]:
    if nm in variants and nm not in top_names:
        top_names.append(nm)

for name in top_names[:16]:
    ...
    print("saved top candidate", out, "ones", int(variants[name].sum()), "pos_ratio", float(variants[name].mean()))
