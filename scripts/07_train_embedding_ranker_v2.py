...
RUN_NAME = "embedding_ranker_v2_catboost_text"
...
def main() -> None:
    ...
    print("=" * 100)
    print("07 TRAIN EMBEDDING RANKER V2")
    print("=" * 100)
    ...
    test["final_score"] = (
        0.70 * test["embedding_rank_score"].astype(np.float32)
        + 0.30 * test["proba"].astype(np.float32)
    ).astype(np.float32)
    ...
    joblib.dump(
        {
            ...
            "run_name": RUN_NAME,
            "task_type": task_type,
            "blend": "final_score = 0.70 * embedding_rank_score + 0.30 * proba",
        },
        meta_path,
    )
