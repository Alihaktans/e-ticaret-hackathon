...
OUT_REVIEW = REPORT_DIR / "review_v44_guarded_v43_swaps.csv"
OUT_SUMMARY = REPORT_DIR / "v44_guarded_v43_candidate_summary.csv"
OUT_EVAL = REPORT_DIR / "v44_guarded_v43_candidate_eval.csv"
OUT_SAVED = REPORT_DIR / "v44_guarded_v43_saved_candidates.csv"
...
def find_query_brands(query_norm, brand_list, max_hits=3):
    hits = []
    q = f" {query_norm} "
...
def semantic_veto(row):
    ...
    dropset = tokset(drop_text)

    reasons = []

    # 1) Brand hard constraint when query contains a known brand.
    ...
    # child/adult mismatch mostly for shoes/clothing/toys.
    if qg["child"] and not ag["child"] and dg["child"]:
        reasons.append("child_query_drop_child_better")
    if not qg["child"] and ag["child"] and (qset & (FOOTWEAR_TYPES | APPAREL_TYPES)):
    ...
    if qnums:
        ...
        # If add misses all query numbers, veto. If drop carried them, stronger.
    ...
    if add_title_ov <= 0.12 and add_cat_ov >= 0.30 and not qbrands:
        reasons.append("category_only_add")

    if add_title_ov <= 0.10 and row["v43_query_pop_support_add"] <= 0.18:
        reasons.append("weak_title_and_pop_support")

    # 5) Some known bad generic intent traps.
    # Query says apple saat: non-Apple smartwatch/kordon is often wrong.
    if "apple" in qset and "saat" in qset and not ("apple" in addset):
        reasons.append("apple_watch_brand_mismatch")

    # Query asks exact tire size.
    if {"r16", "r17", "r18", "r19"} & qset:
        for t in {"r16", "r17", "r18", "r19"} & qset:
            if t not in addset:
...
def semantic_keep(row):
    ...
    title_gain_ok = row["add_title_overlap"] >= row["drop_title_overlap"] + 0.12
    title_abs_ok = row["add_title_overlap"] >= 0.25
    support_ok = row["v43_query_pop_support_add"] >= 0.25
    prob_ok = row["prob_gain"] >= 0.50
    ...
    if title_abs_ok and prob_ok and support_ok and number_ok:
        return True

    if qbrands and brand_ok and support_ok and number_ok and row["add_title_overlap"] >= 0.15:
        return True

    # For very generic queries like "kase", "tepsi", category match can be enough.
    q_len = len(toks(query))
    if q_len <= 2 and row["add_category_overlap"] >= 0.50 and row["prob_gain"] >= 0.80:
        return True
...
def enrich_swaps(sw, item_meta, terms, brand_list):
    ...
    out["v44_veto_reasons"] = out.apply(semantic_veto, axis=1)
    out["v44_veto_reason_text"] = out["v44_veto_reasons"].map(lambda xs: "|".join(xs))
    out["v44_veto"] = out["v44_veto_reasons"].map(lambda xs: len(xs) > 0).astype(int)
    out["v44_semantic_keep"] = out.apply(semantic_keep, axis=1).astype(int)

    out["v44_guard_score"] = (
        out["v43_swap_support_score"].astype(float)
        + 0.30 * out["v44_semantic_keep"].astype(float)
        - 0.35 * out["v44_veto"].astype(float)
        + 0.18 * (out["add_title_overlap"] - out["drop_title_overlap"]).astype(float)
        + 0.08 * out["v43_query_pop_support_add"].astype(float)
    )

    return out.sort_values(["v44_guard_score", "v43_swap_support_score"], ascending=False).reset_index(drop=True)
...
def make_pred_from_swaps(sample, anchor, sw, cap=None, require_keep=True, allow_no_veto=True):
    take = sw.copy()
    if require_keep:
        take = take[take["v44_semantic_keep"].eq(1)].copy()
    if allow_no_veto:
        take = take[take["v44_veto"].eq(0)].copy()

    take = take.sort_values(["v44_guard_score", "v43_swap_support_score"], ascending=False)
    if cap is not None:
...
def main():
    ...
    for target in TARGET_VARIANTS:
        ...
        review_parts.append(enr.sort_values(["v44_veto", "v43_swap_support_score"], ascending=[False, False]).head(250))

        veto_rate = float(enr["v44_veto"].mean()) if len(enr) else 0.0
        keep_rate = float(enr["v44_semantic_keep"].mean()) if len(enr) else 0.0
        clean = enr[(enr["v44_veto"].eq(0)) & (enr["v44_semantic_keep"].eq(1))].copy()
        ...
        for cap in caps:
            ...
            if len(take) == 0:
                continue
            name = f"v44_guarded_{short_hash(target)}_cap{cap}"
            variants[name] = pred
            meta_rows.append({
                "variant": name,
                "source": "v44_guarded_v43",
                ...
                "guard_score_mean": float(take["v44_guard_score"].mean()),
        ...
        for cap in [1200, 2200, 4200]:
            ...
            if len(take) == 0:
                continue
            name = f"v44_noveto_{short_hash(target)}_cap{cap}"
            variants[name] = pred
            meta_rows.append({
                "variant": name,
                "source": "v44_noveto_v43",
                "source_variant": target,
                "cap": cap,
                "used_swaps": int(len(take)),
                "source_swaps": int(len(enr)),
    ...
    if review_parts:
        ...
        front = [
            ...
            "v44_veto", "v44_semantic_keep", "v44_veto_reason_text",
            "v44_guard_score", "v43_swap_support_score", "prob_gain",
    ...
    summary["v44_decision_score"] = summary["weighted_macro"].fillna(0) + move_bonus + guard_bonus + title_bonus - too_big_penalty
    summary = summary.sort_values(["v44_decision_score", "weighted_macro"], ascending=False)
    summary.to_csv(OUT_SUMMARY, index=False)

    save_names = []
    top = summary[
        (summary["source"].isin(["v44_guarded_v43", "v44_noveto_v43"]))
    ...
    for i, nm in enumerate(save_names[:50], 1):
        pred = variants[nm]
        out = SUB_DIR / f"FINAL_CANDIDATE_v44_guarded_{i:03d}_{short_hash(nm)}.csv"
    ...
    print("\nTOP V44")
    cols = [
        "variant", "v44_decision_score", "weighted_macro", "source", "source_variant", "cap",
    ...
    if len(sdf):
        print(sdf[["variant", "file", "v44_decision_score", "weighted_macro", "diff_vs_anchor"]].to_string(index=False))
